...
import { addMessage, clearMessages, renderAttachmentPreview, updateInfobar, showTypingIndicator, hideTypingIndicator, updateTypingStatus } from './renderer.js';
import { addSystemMessage, translateGatewayError } from './utils.js';
import { humanizeError } from './errors.js';
import { updateConnectionUI } from './connection-ui.js';
import { getActiveModelInfo } from './rate-limits.js';
import { getActiveModelUploadPolicy, imageUploadBlockedMessage } from './model-capabilities.js';

import { handleChatEvent, handleAgentEvent, trackRun } from './chat-handler.js';
...
function handleResponse(msg) {
    ...
    if (msg.id && (msg.id.startsWith('usage-status') || msg.id.startsWith('usage-cost'))) {
        import('./usage-data.js').then(m => m.handleUsageRpcResponse(msg));
        return;
    }
