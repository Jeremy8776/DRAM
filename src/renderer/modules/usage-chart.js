...
let lastChartOptions = { width: 0, height: 0, maxCost: 0, stepX: 0, hasData: false };
...
export function renderUsageChart(dailyData) {
    ...
    // Horizontal spacing: distribute points across width
    const stepX = chartData.length > 1 ? width / (chartData.length - 1) : 0;

    lastChartOptions = { width, height, maxCost, stepX, hasData };
    ...
    if (!canvas.dataset.hasListener) {
        canvas.addEventListener('mousemove', (e) => {
            ...
            lastChartData.forEach((_, i) => {
                const px = i * lastChartOptions.stepX;
...
function draw(ctx, data, hoverIndex) {
    const { width, height, maxCost, stepX, hasData } = lastChartOptions;
    const styles = getComputedStyle(document.documentElement);
    const accent = styles.getPropertyValue('--accent').trim() || '#7c3aed';
    const textPrimary = styles.getPropertyValue('--text-primary').trim() || '#fff';
    const textDim = styles.getPropertyValue('--text-tertiary').trim() || '#666';
    const gridMajor = 'rgba(226, 226, 231, 0.12)';
    const gridMinor = 'rgba(226, 226, 231, 0.06)';
    const plotBorder = 'rgba(226, 226, 231, 0.20)';

    const topMargin = 24; // Keep Peak label outside the grid area.
    const bottomMargin = 24;
    const chartHeight = height - topMargin - bottomMargin;
    const plotTop = topMargin;
    const plotBottom = height - bottomMargin;
    const gridCell = 28; // Equal x/y spacing for square grid cells.
    const lineVerticalPadding = gridCell * 0.5;
    ...
    // Plot border for stronger chart framing.
    ctx.strokeStyle = plotBorder;
    ctx.lineWidth = 1;
    ctx.strokeRect(0.5, plotTop + 0.5, width - 1, (plotBottom - plotTop) - 1);
    ...
    points.forEach((p, i) => {
        ...
        if (hasData && i % labelInterval === 0 && hoverIndex === -1) {
            ctx.fillStyle = textDim;
            ctx.font = '10px monospace';
            ctx.textAlign = 'center';
            ctx.fillText(data[i].date.slice(5), p.x, height - 7);
        }
    });
    ...
    ctx.textAlign = 'left';
    if (hasData) {
        ctx.fillText(`Peak: $${maxCost.toFixed(4)}`, 10, 20);
    } else {
        ctx.fillText('Peak: $0.0000', 10, 20);
    }
}
