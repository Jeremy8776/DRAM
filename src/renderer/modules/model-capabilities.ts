...
const TRANSPORT_MAX_REQUEST_BYTES = 5_000_000;
const TRANSPORT_REQUEST_HEADROOM_BYTES = 350_000;
const TRANSPORT_SAFE_REQUEST_BYTES = Math.max(3_000_000, TRANSPORT_MAX_REQUEST_BYTES - TRANSPORT_REQUEST_HEADROOM_BYTES);
const TRANSPORT_RESERVED_OVERHEAD_BYTES = 220_000;
const TRANSPORT_SAFE_IMAGE_BYTES = Math.max(
    MIN_REASONABLE_IMAGE_BYTES,
    Math.floor((TRANSPORT_SAFE_REQUEST_BYTES - TRANSPORT_RESERVED_OVERHEAD_BYTES) * 0.75)
);
const TRANSPORT_SAFE_TOTAL_IMAGE_BYTES = Math.max(
    TRANSPORT_SAFE_IMAGE_BYTES,
    Math.floor((TRANSPORT_SAFE_REQUEST_BYTES - 120_000) * 0.75)
);
...
function resolveActiveModel() {
    const active = (getActiveModelInfo?.() || {}) as { id?: string; name?: string };
...
export async function getActiveModelUploadPolicy() {
    ...
    if (!modelId) {
        return {
            ...
            modelName,
            source: 'none',
            maxImageBytes: Math.min(DEFAULT_IMAGE_BYTES, TRANSPORT_SAFE_IMAGE_BYTES),
            maxTotalImageBytes: Math.min(DEFAULT_TOTAL_IMAGE_BYTES, TRANSPORT_SAFE_TOTAL_IMAGE_BYTES),
            maxAttachments: DEFAULT_MAX_ATTACHMENTS,
            maxImageDimension: DEFAULT_MAX_IMAGE_DIMENSION,
            transportMaxRequestBytes: TRANSPORT_MAX_REQUEST_BYTES,
            transportSafeRequestBytes: TRANSPORT_SAFE_REQUEST_BYTES
        };
    }
    ...
    const providerImageBytes = clamp(
        Math.floor(maxImageBytesDetected || providerDefaults.maxImageBytes),
        MIN_REASONABLE_IMAGE_BYTES,
        MAX_REASONABLE_IMAGE_BYTES
    );
    const providerTotalImageBytes = clamp(
        Math.floor(maxTotalImageBytesDetected || Math.max(providerDefaults.maxTotalImageBytes, providerImageBytes * 2)),
        providerImageBytes,
        MAX_REASONABLE_IMAGE_BYTES * 3
    );
    const maxImageBytes = Math.min(providerImageBytes, TRANSPORT_SAFE_IMAGE_BYTES);
    const maxTotalImageBytes = Math.min(
        Math.max(maxImageBytes, providerTotalImageBytes),
        TRANSPORT_SAFE_TOTAL_IMAGE_BYTES
    );
