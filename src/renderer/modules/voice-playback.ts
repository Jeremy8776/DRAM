 ...
 */
export async function queueVoiceResponse(
    text,
    onPlay,
    params: VoicePlaybackParams = {}
) {
...
async function processAudioQueue(params) {
    const { mediaRecorder, updateStatus = () => { } } = params as VoicePlaybackParams;
...
async function playAudioData(dataUri, onPlay, params) {
    const { audioContext, analyser } = params as VoicePlaybackParams;
