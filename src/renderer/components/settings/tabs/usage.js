...
export function updateUsageStats(stats) {
    ...
    if (listContainer) {
        const paidBreakdown = Array.isArray(stats.breakdown)
            ? stats.breakdown
            : Object.entries(stats.providers || {}).map(([id, data]) => ({
                id,
                provider: id,
                label: data?.displayName || id,
                ...data
            }));
        const localBreakdown = Array.isArray(stats.localBreakdown) ? stats.localBreakdown : [];
 ...
 */
function renderBreakdownCard(item, options = {}) {
