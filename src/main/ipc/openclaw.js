/**
 * DRAM IPC - OpenClaw Discovery & Management
 * Handles detection, installation, and lifecycle of external OpenClaw
 */
...
// Common npm global paths to check
...
export class OpenClawManager {
  ...
  /**
   * Main discovery - find OpenClaw installation
   */
  async discover() {
    ...
    // 1. Check if CLI is available (bundled or global)
    const cliCheck = await this._findCli();
    if (cliCheck.found) {
      result.installed = true;
      result.cliPath = cliCheck.path;
      result.source = cliCheck.source; // 'bundled' or 'global'
    ...
    if (configInfo) {
      ...
      // Check if config has meaningful user settings (not just defaults)
      // A "fresh" config won't have agents.defaults.model.primary set
    ...
    if (result.source === 'global' && result.found) {
      // User has global OpenClaw with meaningful config - show detection
      ...
      // User has global OpenClaw but empty config - fresh install scenario
      this.debugLog('[OpenClaw] Global OpenClaw but empty config - fresh install');
      result.needsInstall = false; // Already installed globally
    } else if (!result.installed) {
      // Nothing found anywhere - need to install
      this.debugLog('[OpenClaw] Not found - needs install');
      result.needsInstall = true;
    }

    // 4. Start watching config for external changes
  ...
  /**
   * Find OpenClaw CLI - only checks global installations
   * Returns { found: boolean, path: string|null, source: 'global'|null }
   */
  async _findCli() {
    // 1. Check npm global - user installed separately
    try {
  ...
  /**
   * Check if config has meaningful user settings (not just defaults/empty)
   * Returns true if user has configured models, workspace, or plugins
   */
  _hasMeaningfulConfig(config) {
    if (!config || typeof config !== 'object') return false;

    // Get model and strip provider prefix if present (e.g., "anthropic/claude-3-7-sonnet-latest" -> "claude-3-7-sonnet-latest")
    ...
    // Check for primary model selection (not just the default)
    ...
    // Check for custom workspace (not the default Documents/DRAM)
    ...
    // Check for custom gateway settings (not auto-generated token)
  ...
  /**
   * Start watching config file for external changes (CLI usage)
   */
  ...
  /**
   * Get available OpenClaw versions from npm
   */
  ...
  /**
   * Write config (preserves OpenClaw format)
   */
...
/**
 * Register IPC handlers for OpenClaw management
 */
export function registerOpenClawHandlers(ipc, _stateManager, windowManager, debugLog) {
  ...
  // Engine initialization after OpenClaw install
