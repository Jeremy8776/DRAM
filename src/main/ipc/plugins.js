...
export function registerPluginHandlers(ipc, internalRequest) {
    const normalizeQrStartResult = (data = {}) => {
    ...
    const normalizeQrWaitResult = (data = {}) => {
     ...
     */
    ipc.handle('util:whatsappStartLogin', async (_event, opts = {}) => {
        try {
            const params = { ...(opts || {}) };
     ...
     * NOTE: In symbiotic mode, this uses the OpenClaw gateway instead of bundled modules
     */
    ipc.handle('util:whatsappPollLogin', async (_event, opts = {}) => {
        try {
            const params = { ...(opts || {}) };
            delete params.channel;
     ...
     */
    ipc.handle('util:webLoginStart', async (_event, opts = {}) => {
        try {
            const params = { ...(opts || {}) };
            delete params.channel;
            const result = await internalRequest('web.login.start', params);
     ...
     */
    ipc.handle('util:webLoginWait', async (_event, opts = {}) => {
        try {
            const params = { ...(opts || {}) };
            delete params.channel;
            const result = await internalRequest('web.login.wait', params);
